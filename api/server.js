...
try {
  ...
  // Template Editor Authentication Middleware
  const templateEditorAuth = (req, res, next) => {
    const accessKey = req.headers.authorization || '';
    
    // Check if the access key matches the special key
    if (accessKey === 'iamunique7$') {
      next(); // Allow access
    } else {
      log('Unauthorized template editor access attempt');
      res.status(401).json({ 
        success: false, 
        error: 'Unauthorized',
        message: 'You need a valid access key to access the template editor'
      });
    }
  };
  ...
  app.get('/api/templates', async (req, res) => {
    try {
      log('GET /api/templates - Fetching all templates');
      
      // Read templates from the file system
      const templatesDir = path.join(process.cwd(), 'src', 'templates');
      let templates = [];
      
      // Check if the directory exists
      if (fs.existsSync(templatesDir)) {
        const frameworkDirs = fs.readdirSync(templatesDir).filter(file => 
          fs.statSync(path.join(templatesDir, file)).isDirectory() && 
          !file.startsWith('.')
        );
        
        // Iterate through framework directories (react, nextjs, etc.)
        for (const framework of frameworkDirs) {
          if (framework !== 'node_modules' && !framework.startsWith('.')) {
            const frameworkPath = path.join(templatesDir, framework);
            
            // Get all template directories within this framework
            const templateDirs = fs.readdirSync(frameworkPath).filter(file => 
              fs.statSync(path.join(frameworkPath, file)).isDirectory() && 
              !file.startsWith('.')
            );
            
            // Read each template configuration
            for (const templateDir of templateDirs) {
              const templateJsonPath = path.join(frameworkPath, templateDir, 'template.json');
              
              if (fs.existsSync(templateJsonPath)) {
                try {
                  const templateData = JSON.parse(fs.readFileSync(templateJsonPath, 'utf8'));
                  templateData.relativePath = `${framework}/${templateDir}`;
                  templates.push(templateData);
                } catch (err) {
                  log(`Error parsing template.json for ${templateDir}: ${err.message}`);
  ...
  // Protected endpoints that require the special access key
  
  // Add a new template
  app.post('/api/templates', templateEditorAuth, async (req, res) => {
    try {
      log('POST /api/templates - Adding new template');
      const templateData = req.body;
      
      // Validate required fields
      if (!templateData.id || !templateData.name || !templateData.framework) {
        return res.status(400).json({
          success: false,
          error: 'Missing required fields',
          message: 'Template ID, name, and framework are required'
        });
      }
      
      // Ensure template ID is URL-friendly
      const sanitizedId = templateData.id.toLowerCase().replace(/[^a-z0-9-]/g, '-');
      
      // Create directory structure for the new template
      const templatesDir = path.join(process.cwd(), 'src', 'templates');
      const frameworkDir = path.join(templatesDir, templateData.framework);
      const templateDir = path.join(frameworkDir, sanitizedId);
      ...
      if (fs.existsSync(templateDir)) {
        return res.status(400).json({
          ...
          message: `A template with ID '${sanitizedId}' already exists`
        });
      }
      
      fs.mkdirSync(templateDir, { recursive: true });
      
      // Create src directory inside template directory
      ...
      // Create a placeholder preview image if not provided
      if (!templateData.imageFile) {
        // Copy a default preview image
        const defaultImage = path.join(process.cwd(), 'public', 'placeholder-template.png');
        if (fs.existsSync(defaultImage)) {
          fs.copyFileSync(defaultImage, path.join(templateDir, 'preview.png'));
        }
      }
      
      log(`Template '${sanitizedId}' created successfully`);
      res.status(201).json({
        success: true,
  ...
  app.put('/api/templates/:id', templateEditorAuth, async (req, res) => {
    try {
      const { id } = req.params;
      const templateData = req.body;
      
      log(`PUT /api/templates/${id} - Updating template`);
      ...
      // Search for the template in all framework directories
      const frameworkDirs = fs.readdirSync(templatesDir).filter(file => 
        fs.statSync(path.join(templatesDir, file)).isDirectory() && 
        !file.startsWith('.')
      );
      ...
      for (const framework of frameworkDirs) {
        const possiblePath = path.join(templatesDir, framework, id);
      ...
      if (fs.existsSync(templateJsonPath)) {
        existingData = JSON.parse(fs.readFileSync(templateJsonPath, 'utf8'));
      }
      
      // Merge existing data with new data
      const updatedData = { ...existingData, ...templateData };
      
      // Save updated template.json
      fs.writeFileSync(
        templateJsonPath,
        JSON.stringify(updatedData, null, 2)
      );
      
      log(`Template '${id}' updated successfully`);
      res.json({
        success: true,
        templateId: id,
        message: `Template '${templateData.name || id}' updated successfully`
      });
    } catch (error) {
      log(`Error updating template: ${error.message}`);
      res.status(500).json({
        success: false,
  ...
  // Delete a template
  app.delete('/api/templates/:id', templateEditorAuth, async (req, res) => {
    try {
      const { id } = req.params;
      log(`DELETE /api/templates/${id} - Deleting template`);
      ...
      log(`Template '${id}' deleted successfully`);
      res.json({
        success: true,
